...
import { Home, Settings, GraduationCap, Box, File, UserCog, Users2, Contact, ChevronLeft, ChevronRight, User, LogOut } from 'lucide-react'
    ...
    <>
      ...
      >
        ...
        <nav className={`flex-1 overflow-y-auto ${isCollapsed ? 'p-2' : 'p-4'}`}>
          ...
          <div className='mt-6'>
            {!isCollapsed && (
              <h3 className='text-sm font-semibold text-emerald-300 uppercase tracking-wider mb-3'>Plataforma</h3>
            )}
            <Link
              to="/admin/platform/members-area"
              className={`flex items-center gap-3 rounded-lg hover:bg-emerald-800 transition-colors mb-2 ${
                isCollapsed ? 'justify-center p-2' : 'p-3'
              }`}
              activeProps={{
            ...
            >
              <UserCog size={20} />
              {!isCollapsed && <span className="font-medium">Área de Membros</span>}
            </Link>
